...
export default function PayForm() {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      if (!file.type.startsWith("image/")) {
        ...
        setAlertMessage({ type: 'error', message: 'Please upload an image file (JPEG, PNG, etc.)' });
        return;
      }
      ...
      setAlertMessage({ type: 'success', message: 'Image uploaded successfully' });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setAlertMessage({ type: 'info', message: 'Submitting payment details...' });
      ...
      setAlertMessage({ 
        type: 'error', 
        message: 'Error submitting payment details. Please check your connection and try again.' 
      });
    }
  };
      ...
      <div className="max-w-lg mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
        ...
        {alertMessage.message && (
          <div className={`mb-4 p-4 rounded-md ${
            alertMessage.type === 'error' ? 'bg-red-100 text-red-700' :
            alertMessage.type === 'success' ? 'bg-green-100 text-green-700' :
            alertMessage.type === 'info' ? 'bg-blue-100 text-blue-700' :
          ...
          <div>
            ...
            {preview && (
              <div className="mt-3">
                ...
                />
                <p className="text-sm text-gray-500 mt-1">Preview of uploaded image</p>
              </div>
            )}
          </div>
